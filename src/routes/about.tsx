...
export const Route = createFileRoute("/about")({
  head: () => ({ meta: [{ title: "About — BlogiFy" }, { name: "description", content: "About BlogiFy and Jalal Nasser." }] }),
  component: () => <StaticPage slug="about" />,
});

function StaticPage({ slug }: { slug: string }) {
  const { data: page } = useQuery({ queryKey: ["page", slug], queryFn: () => fetchPageBySlug(slug) });
  return (
